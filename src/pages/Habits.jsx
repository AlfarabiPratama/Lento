...
import { IconPlus, IconX, IconFlame, IconCheck, IconChevronDown, IconPencil } from '@tabler/icons-react'
...
function Habits() {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            if (editingHabit) {
                ...
                await update(editingHabit.id, habitForm)
                showToast('success', `Kebiasaan "${habitForm.name}" berhasil diupdate`)
            } else {
                // Create new habit
                await create(habitForm)
                showToast('success', `Kebiasaan "${habitForm.name}" berhasil ditambahkan`)
            }
    ...
    const handleEdit = (habit) => {
        ...
        setHabitForm({
            ...
            reminder_days: habit.reminder_days || null
        })
        setShowForm(true)
    }
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3 p-3 rounded-xl border border-line">
                            <label className="flex items-center gap-2 cursor-pointer flex-1">
                                <input
                                    type="checkbox"
                                    checked={habitForm.reminder_enabled || false}
                                    onChange={async (e) => {
                            ...
                            {habitForm.reminder_enabled && (
                                <input
                                    type="time"
                                    value={habitForm.reminder_time || '08:00'}
                                    onChange={(e) => setHabitForm(prev => ({ ...prev, reminder_time: e.target.value }))}
                        ...
                        {habitForm.reminder_enabled && (
                            <details className="group">
                                ...
                                <div className="mt-3 pl-6">
                                    <WeekdaySelector
                                        value={habitForm.reminder_days || []}
                                        onChange={(days) => setHabitForm(prev => ({
                                            ...prev,
                    ...
                    {habits.map((habit) => (
                        ...
                        >
                            ...
                            >
                                ...
                                {/* Edit button - icon 20px */}
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation()
                                        handleEdit(habit)
                                    }}
                                className="opacity-0 group-hover:opacity-100 min-w-11 min-h-11 flex items-center justify-center rounded-lg text-ink-muted hover:text-primary hover:bg-primary/10 transition-all"
